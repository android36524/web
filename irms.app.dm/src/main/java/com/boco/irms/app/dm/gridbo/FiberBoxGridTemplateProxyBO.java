...
public class FiberBoxGridTemplateProxyBO extends XmlTemplateGridBO {
	...
	public PageResult getGridData(PageQuery queryParam, GridCfg param) {
        ...
        //拼装查询条件
		String sql = getSql(param,className,name);
	...
	public PageResult getGridPageInfo(PageQuery queryParam, GridCfg param) {
		...
		String className = editorMeta.getClassName();
        //拼装查询条件
		String sqlcon = getSql(param,className,name);
     ...
     * @return
     */
    public String getSql(GridCfg param,String className,String type){
    	String sql =" 1= 1";
    	
    	if(type != null && !type.equals("")){
    		if(type.equals("IRMS.RMS.FIBERJOINTBOX")){
    			sql += " AND KIND = 1 ";
    		}else if(type.equals("IRMS.RMS.FIBERTERMINALBOX")){
    			sql += " AND KIND = 2 ";
    		}
    	}
