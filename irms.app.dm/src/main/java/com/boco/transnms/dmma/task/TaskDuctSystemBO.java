...
public class TaskDuctSystemBO extends XmlTemplateGridBO  implements DmmaCommonIntf<ITaskBO>{
	...
	protected String getSQL( GridCfg arg1) {
	      String sql = "SELECT * FROM DUCT_SYSTEM WHERE 1=1 ";
	      if(arg1 != null && arg1.getCfgParams() != null && arg1.getCfgParams().get("queryData") != null){
	    	  ...
	    	  if (StringUtils.isNotEmpty(labelcn) && !StringUtils.equalsIgnoreCase(labelcn, "全部")) {
	              sql += " AND LABEL_CN like '%" + labelcn + "%' ";
	          };
	          if (StringUtils.isNotEmpty(project) && !StringUtils.equalsIgnoreCase(project, "全部")) {
	              sql += " AND  PROJECT like '%" + project + "%' ";
	          };
	          if (StringUtils.isNotEmpty(ownership) && !StringUtils.equalsIgnoreCase(ownership, "全部")) {
	              sql += " AND OWNERSHIP=" + ownership + " ";
	          }
	          if (StringUtils.isNotEmpty(systemLevel) && !StringUtils.equalsIgnoreCase(systemLevel, "全部")) {
	              sql += " SYSTEM_LEVEL=" + systemLevel + " ";
	          }
	          if (StringUtils.isNotEmpty(mainMode) && !StringUtils.equalsIgnoreCase(mainMode, "全部")) {
		...
		private String getCountSql( GridCfg arg1){
			 String sql = " SELECT COUNT(*) COUNTVALUE FROM DUCT_SYSTEM WHERE 1=1 ";  
			  if(arg1 != null && arg1.getCfgParams() != null && arg1.getCfgParams().get("queryData") != null){
		    	  JSONObject jsonObj = JSONObject.fromObject( arg1.getCfgParams().get("queryData") );
		    	  String  labelcn = jsonObj.getString("LABEL_CN");
		    	  String  project = jsonObj.getString("PROJECT");
