...
Frame.grid.plugins.tbar.HangWallGridTbar = Ext.extend(Object,{
	constructor : function(grid) {
		this.grid = grid;
		Frame.grid.plugins.tbar.HangWallGridTbar.superclass.constructor.call(this);
		var isShowCAD = true;
	...
	_export:function(){
		var url = ctx+"/dm/export.do?name="+encodeURI(encodeURI("挂墙系统导入"));
		window.open(url);
	},
	...
	_exportseg:function(){
		var url = ctx+"/dm/export.do?name="+encodeURI(encodeURI("挂墙段"));
		window.open(url);
	},
	_roteManage : function() {
		var records = this.getSelectionModel().getSelections();
		if (Ext.isEmpty(records) || records.length == 0) {
			Ext.Msg.alert('温馨提示', '请选择挂墙系统数据.');
			return;
		}
		if (records.length > 1) {
			Ext.Msg.alert('温馨提示', '目前只支持单条挂墙系统的路由管理.');
			return;
		}
	...
	_method1 : function() {
		...
		Ext.MessageBox.show({
			...
			fn : function(btn) {
				if (btn == "ok") {
					CalculateSystemLengthAction.doHWCalculateSystemLength(cuid);
					Ext.MessageBox.show({
						msg : '计算中,请等待..',
						progressText : '计算中...',
						width : 300,
	...
	exportFile : function() {
		...
		GridDataExport.exportData('code=service_dict_export_dm.DM_EXPORT_HANGWALL_SEG&c_paramKey=RELATED_SYSTEM_CUID');
	},
