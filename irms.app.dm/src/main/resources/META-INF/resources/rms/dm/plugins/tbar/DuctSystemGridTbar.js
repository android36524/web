...
Frame.grid.plugins.tbar.DuctSystemGridTbar = Ext.extend(Object,{
	constructor : function(grid) {
		this.grid = grid;
		Frame.grid.plugins.tbar.DuctSystemGridTbar.superclass.constructor.call(this);
		var isShowCAD = true;
	...
	_exportCAD : function() {
		...
		if (records.length > 1) {
			Ext.Msg.alert('温馨提示', '目前只支持单条管道系统的CAD导出.');
			return;
		}
		var cuid = records[0].data['CUID'];
	...
	_routeManage : function() {
		...
		if (records.length > 1) {
			Ext.Msg.alert('温馨提示', '目前只支持单条管道系统的路由管理.');
			return;
		}
		...
		FrameHelper.openUrl(ctx + url + '&cuid=' + cuid
				+ "&objectId=" + objectId, labelCn + '路由管理');
	},
	_routeTopo : function() {
		var records = this.getSelectionModel().getSelections();
		if (Ext.isEmpty(records) || records.length == 0) {
		...
		var cuid = records[0].data['CUID'];
		var url = ctx
				+ "/topo/index.do?code=DuctRouteSectionTopo&resId="
				+ cuid
				+ "&resType=DUCT_SYSTEM&clientType=html5";
		FrameHelper.openUrl(url, '管道路由图-'
				+ records[0].data['LABEL_CN']);
	},
	_method1 : function() {
	...
	exportFile : function() {
		...
		GridDataExport
				.exportData('code=service_dict_export_dm.DM_EXPORT_DUCT_SEG&c_paramKey=RELATED_SYSTEM_CUID');
	},
