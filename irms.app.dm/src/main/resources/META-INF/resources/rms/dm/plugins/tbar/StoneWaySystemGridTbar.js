...
Frame.grid.plugins.tbar.StoneWaySystemGridTbar = Ext.extend(Object,{
	constructor : function(grid) {
		this.grid = grid;
		Frame.grid.plugins.tbar.StoneWaySystemGridTbar.superclass.constructor.call(this);
		var isShowCAD = true;
	...
	_export:function(){
		var url = ctx+"/dm/export.do?name="+encodeURI(encodeURI("标石路由系统导入"));
		window.open(url);
	},
	...
	_exportseg:function(){
		var url = ctx+"/dm/export.do?name="+encodeURI(encodeURI("标石路由段"));
		window.open(url);
	},
	...
	_roteTopo : function() {
	    ...
	    if(Ext.isEmpty(records) || records.length==0){
		 ...
		 var cuid=records[0].data['CUID'];
		  var url = ctx+"/topo/index.do?code=DuctRouteSectionTopo&resId="+cuid+"&resType=STONEWAY_SYSTEM&clientType=html5";
		 
		 FrameHelper.openUrl(url,'标石路由图-'+records[0].data['LABEL_CN']);						 
	},
	_method1 : function() {
		var records=this.getSelectionModel().getSelections();
		if(Ext.isEmpty(records)||records.length==0){
	...
	exportFile : function() {
		...
		GridDataExport.exportData('code=service_dict_export_dm.DM_EXPORT_STONEWAY_SEG&c_paramKey=RELATED_SYSTEM_CUID');
	},
