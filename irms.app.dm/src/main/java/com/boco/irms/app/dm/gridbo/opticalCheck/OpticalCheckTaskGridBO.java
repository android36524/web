...
import com.boco.irms.app.dm.gridbo.GridTemplateProxyBO;
import com.boco.transnms.client.model.base.BoCmdFactory;
import com.boco.transnms.common.dto.base.BoQueryContext;
import com.boco.transnms.common.dto.base.DboCollection;
import com.boco.transnms.common.dto.base.GenericDO;

public class OpticalCheckTaskGridBO extends GridTemplateProxyBO {
	...
	public PageResult getGridData(PageQuery queryParam, GridCfg param) {
		...
		try {
			results = (DboCollection) BoCmdFactory.getInstance().execBoCmd(method, 
					querycon, relatedSystemCuid,param.getCfgParams().get("taskType"));//IOpticalCheckBO.getOCRByTaskcuidAndType
		} catch (Exception e) {
		...
		if (results != null && results.size() > 0) {
			for (int i = 0; i < results.size(); i++) {
				GenericDO gdo = results.getAttrField("OPTICAL_CHECK_RESULT", i);
				...
				gdo.setAttrValue("TASK_NAME",param.getCfgParams().get("taskName"));
				Map map = new HashMap();
