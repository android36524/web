...
Frame.grid.plugins.tbar.SubProjectManagementGridTbar = Ext.extend(Object, {

	constructor : function(grid) {
		this.grid = grid;
		Frame.grid.plugins.tbar.SubProjectManagementGridTbar.superclass.constructor.call(this);
	...
	_relatedResources: function() {
		...
		if(Ext.isEmpty(records) || records.length==0 || records.length>1){
			 Ext.Msg.alert('温馨提示','请选择一条项目管理数据操作。');
			 return;
		}else{
	...
	_constructToMaintain : function(){
		var records = this.getSelectionModel().getSelections();
		if(Ext.isEmpty(records) || records.length > 1){
			Ext.Msg.alert('温馨提示','请选择一条项目状态为施工的数据操作');
			return;
		}else{
			var stateValue = records[0].data.STATE.CUID;
			if(Ext.isEmpty(stateValue) || stateValue != "2"){
				Ext.Msg.alert('温馨提示','项目状态不是施工，该按钮不可操作');
				return;
			}else{
				this.formPanel = new DM.form.formPamel({});
				var win = WindowHelper.openExtWin(this.formPanel,{
	...
	_maintainToAccept : function(){
		var records = this.getSelectionModel().getSelections();
		if(Ext.isEmpty(records) || records.length > 1){
			Ext.Msg.alert('温馨提示','请选择一条项目状态为交维的数据操作');
			return;
		}else{
			var stateValue = records[0].data.STATE.CUID;
			if(Ext.isEmpty(stateValue) || stateValue != "3"){
				Ext.Msg.alert('温馨提示','项目状态不是交维，该按钮不可操作');
				return;
			}else{
				this.formPanel = new DM.form.formPamel({});
				var win = WindowHelper.openExtWin(this.formPanel,{
	...
	_acceptProject : function(){
		var records = this.getSelectionModel().getSelections();
		if(Ext.isEmpty(records) || records.length > 1){
			Ext.Msg.alert('温馨提示','请选择一条项目状态为待审批的数据操作');
			return;
		}else{
			var stateValue = records[0].data.STATE.CUID;
			if(Ext.isEmpty(stateValue) || stateValue != "5"){
				Ext.Msg.alert('温馨提示','项目状态不是待审批，该按钮不可操作');
				return;
			}else{
