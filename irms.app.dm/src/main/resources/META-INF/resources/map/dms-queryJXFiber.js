$importjs(ctx+'/dwr/interface/JXFiberQueryAction.js');
tp.utils.createJXQueryConPanel = function(treeTableView){
    	...
    	queryBtn.onclick = function(e){
    		var queryParam={'ORIG_DEVICE':origDevice.getResult().id,
                            ...
                            'FIBER_STATUS':fiberStateCombo.getValue(),
		...
		var fiberState = {values:[ -1,1, 2, 3, 4, 5],labels:['全部','未用', '在用', '预占', '坏纤', '不可用纤']};
		var fiberStateCombo = tp.utils.createComboBox(fiberState);
		fiberStateCombo.setValue(-1);

		var transitionPointCount = {values:[0,1,2,3],labels:['0','1','2','3']};
		...
		basicFormPane.addRow([{element:'起点设备：',align:'left'},{element:origDevice,align:'left'},
		                      {element:'光纤使用状态：',align:'left'},fiberStateCombo,null,queryBtn],
		                     [0.1,0.2,0.1,0.2,0.2,80]);
		basicFormPane.addRow([{element:'终点设备：',align:'left'},{element:destDevice,align:'left'},
		                      {element:'转接点个数：',align:'left'},transitionPointCountCombo,null,clearBtn],
			                     [0.1,0.2,0.1,0.2,0.2,80]);
		...
		transitionPointCountCombo.onValueChanged = function() {
			...
			basicFormPane.clear();
		    basicFormPane.addRow([ {element : '起点设备：', align : 'left'}, {element:origDevice,align:'left'}, 
		                           {element : '光纤使用状态：',align : 'left'}, fiberStateCombo, 
		                           {element : '必须经过的转接点：', align : 'left'}, {element:includePoint,align:'left'},null,queryBtn  ],
		                           [0.1,0.2,0.13,0.2,0.16,0.2,10,80]);
		    basicFormPane.addRow([ {element : '终点设备：',align : 'left'}, {element:destDevice,align:'left'},
		                           {element : '转接点个数：',align : 'left'}, transitionPointCountCombo, 
		                           {element : '不能经过的转接点：',align : 'left'},{element:excludePoint,align:'left'},null,clearBtn ], 
		                           [0.1,0.2,0.13,0.2,0.16,0.2,10,80]);
	    };
...
tp.utils.createJXQueryPanel = function (){
	...
	treeTableView.addColumns([{name:"original",displayName:"起点",accessType:"attr"},
	                          {name:"fiberRoute",displayName:"光纤路由",accessType:"attr"},
	                          {name:"level",displayName:"级别",accessType:"attr"},
	                          {name:"destination",displayName:"终点",accessType:"attr"}]);
	
	//TODO 根据配置动态加载查询面板
	var basicFormPane = new tp.utils.createJXQueryConPanel(treeTableView);	
...
tp.utils.doQuery= function (treeTableView,queryParam){
	JXFiberQueryAction.getResourceScheme(queryParam,function(data){
	        ...
	        treeTableView.getView().addEventListener('dblclick', function(e){
                ...
                if(data&&data.a('supportDoubleClick')){
                	JXFiberQueryAction.getOpticals(data._id,data._name,function(optials){
