...
public class PosQueryTemplateProxyBO extends GridTemplateProxyBO {
	...
	public PageResult getGridData(PageQuery queryParam, GridCfg param) {
		...
		try {
			...
			DataObjectList resultsTemp = bo.getObjectsBySql(sql, new AnPos());
			results = getResultByPage(resultsTemp, queryParam);
//			results = (DboCollection)BoCmdFactory.getInstance().execBoCmd(method, querycon,sql);
    ...
    public String getSql(GridCfg param,String className){
    	String sql =" related_cab_cuid is null ";
