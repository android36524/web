...
public class ProjectTaskBO  extends XmlTemplateGridBO  implements DmmaCommonIntf<ITaskBO>{
	
	protected String getSQL(GridCfg arg1) {
		String sql = " select t.*,pd.user_name as pda_user_name,tp.task_state as pda_task_state, " +
				"tp.accept_time as pda_accept_time,tp.actual_end_time as pda_actual_end_time " +
				"from pda_task t,pda_device pd,task_to_pda tp " +
				"where t.task_type=2 and t.cuid=tp.related_task_cuid " +
		...
		if(!"全部".equals(tempTaskState)){
			sql += " and t.task_state="+tempTaskState;
		}
		if(!"全部".equals(tempPdaTaskState)){
			sql += " and tp.task_state="+tempPdaTaskState;
		}
	...
	public PageResult getGridData(PageQuery arg0, GridCfg arg1) {
		...
		DataObjectList data = taskbo.querySubTask(getSQL(arg1), arg0.getCurPageNum(), arg0.getPageSize());
