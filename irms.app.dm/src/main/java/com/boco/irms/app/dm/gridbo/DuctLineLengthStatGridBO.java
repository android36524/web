...
public class DuctLineLengthStatGridBO extends GridTemplateProxyBO {
	...
	private String getSql(GridCfg param,String[] sqlTemplate,String className){
		...
		if(sqlTemplate.length == 1){
			...
			if(cuid != null){
				try {
					String sql = "SELECT RELATED_DISTRICT_CUID,CHECK_MONTH,RESULT_TYPE FROM " + className + " WHERE CUID = '" + cuid + "'";
