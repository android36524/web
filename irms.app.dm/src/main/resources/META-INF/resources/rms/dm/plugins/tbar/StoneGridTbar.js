...
Frame.grid.plugins.tbar.StoneGridTbar = Ext.extend(Object,{
	constructor : function(grid) {
		this.grid = grid;
		Frame.grid.plugins.tbar.StoneGridTbar.superclass.constructor.call(this);
		return [ '-', {
			text : '定位',
	...
	_export:function(){
		var url = ctx+"/dm/export.do?name="+encodeURI(encodeURI("标石导入"));
		window.open(url);
	},
	...
	_batchNames : function() {
		...
		if (this.batchNamesPanel == null) {
			this.batchNamesPanel = new Frame.grid.edit.BatchNamesPanel({postfix:'号标石'});
		}
		var scope = this;
