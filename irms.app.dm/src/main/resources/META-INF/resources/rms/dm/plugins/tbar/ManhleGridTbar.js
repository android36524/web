...
Frame.grid.plugins.tbar.ManhleGridTbar = Ext.extend(Object,{
	constructor : function(grid) {
		this.grid = grid;
		Frame.grid.plugins.tbar.ManhleGridTbar.superclass.constructor.call(this);
		return [ '-', {
			text : '定位',
			...
			iconCls : 'c_page_white_link',
			scope : this,
			handler : this._manhleExpandedView
		} ,'-',{
			text : '查看关联线设施',
			iconCls : 'c_page_white_link',
			scope : this,
	...
	_manhleExpandedView : function() {
	    ...
	    if(Ext.isEmpty(records) || records.length==0){
		 ...
		 var url = ctx+"/topo/index.do?code=ManhleSectionTopo&resId="+cuid+"&resType=MANHLE&clientType=html5";						 
		 FrameHelper.openUrl(url,'人手井展开图-'+records[0].data['LABEL_CN']);						 
	},
	...
	_batchNames : function() {
		    ...
		    if(this.batchNamesPanel==null){
		    	this.batchNamesPanel=new Frame.grid.edit.BatchNamesPanel({postfix:'号井',number:records.length});
		    }
		    var scope=this;
	...
	_export:function(){
		var url = ctx+"/dm/export.do?name="+encodeURI(encodeURI("人手井导入"));
		window.open(url);
	},
