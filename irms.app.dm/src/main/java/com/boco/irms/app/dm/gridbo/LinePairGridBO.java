 ...
 *
 */
public class LinePairGridBO extends GridTemplateProxyBO {

	@SuppressWarnings("rawtypes")
	@Override
	public PageResult getGridData(PageQuery queryParam, GridCfg param) {
		String interCableCuid = param.getCfgParams().get("cuid");
		GenericDO igdo = new GenericDO();
		try {
			igdo = (GenericDO) BoCmdFactory.getInstance()
						.execBoCmd("IDuctManagerBO.getObjByCuid",new BoActionContext(), interCableCuid);
		} catch (Exception e) {
			logger.error("失败");
		}
		// 获取列名
		...
		String method = "ILinePairBO.getInterCableSimpleLinePair";
		...
		try {
			results = (DataObjectList) BoCmdFactory.getInstance().execBoCmd(method, new BoActionContext(), interCableCuid);
		} catch (Exception e) {
		...
		if (results != null && results.size() > 0) {
			for (int i = 0; i < results.size(); i++) {
				...
				gdo.setAttrValue("RELATED_SEG_CUID", igdo);
